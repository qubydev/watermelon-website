  ...
  return (
    <div className="grid gap-4">
      {images.map((image, imageIndex) => (
        <div className="product-noise border border-border/80" key={`${image}-${imageIndex}`}>
          <img
...
export default function Product() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0)
    document.title = product ? `${product.name} Case Study | Watermelon Studio` : 'Product not found | Watermelon Studio'
  }, [product])
  ...
  return (
    <SplitLayout
      ...
      showcase={<ProductGallery product={product} />}
